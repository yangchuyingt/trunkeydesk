...
public class RemoteCanvas extends ImageView implements LibFreeRDP.UIEventListener, LibFreeRDP.EventListener {
    ...
    public RemoteCanvas(final Context context, AttributeSet attrs) {
       ...
       //isSpice = getContext().getPackageName().contains("SPICE");
         ...
         System.out.println("packagename:"+getContext().getPackageName());
    ...
    private void startSpiceConnection() throws Exception {
        ...
        spicecomm = new SpiceCommunicator (getContext(), this, connection);//初始化了视频的播放流
        rfbconn = spicecomm;
        pointer = new RemoteSpicePointer (rfbconn, RemoteCanvas.this, handler);//鼠标的移动
        keyboard = new RemoteSpiceKeyboard (getResources(), spicecomm, RemoteCanvas.this, handler, connection.getLayoutMap());//键盘
        spicecomm.setUIEventListener(RemoteCanvas.this);
        spicecomm.setHandler(handler);
        System.out.println("address:"+address+",port"+port+",tport"+tport+",password:"+connection.getPassword()+",certpath:"+connection.getCaCertPath()+",enableSound:"+connection.getEnableSound());
    ...
    public void OnGraphicsUpdate(int x, int y, int width, int height) {
        ...
        if (isRdp) {
            ...
            synchronized (bitmapData.mbitmap) {
            	//System.out.println("bitmap length:"+bitmapData.mbitmap.getByteCount());
